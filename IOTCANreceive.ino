...
#define VRX_PIN  26       // ESP32 pin GPIO26 connected to VRX pin
#define VRY_PIN  25       // ESP32 pin GPIO25 connected to VRY pin

int valueX = 0;           // to store the X-axis value for joysticks
int valueY = 0;           // to store the Y-axis value for joysticks
...
void sendBack() {
  ...
  // if it received something, send it back
    ...
    // convert these analog values into byte values 
    valueX = byte(valueX);              
    valueY = byte(valueY); 

    // convert these values into separate digits for X and Y 
...
//  Serial.println("s");
  ...
  // prints out the values for testing 
  ...
  delay(1);        // delay in between reads for stability
    ...
    // need to implement a queue for this to ensure both values get sent evenly 
